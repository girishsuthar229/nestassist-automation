...
- Name: api\auth.api.spec.ts >> API – Auth Endpoints (/api/auth) >> POST /api/auth/forgot-password >> [POSITIVE] Registered email → 200 with success message
- Location: tests\api\auth.api.spec.ts:111:9
...
Call log:
  - → POST https://nestassist-backend-production.up.railway.app/api/auth/forgot-password
